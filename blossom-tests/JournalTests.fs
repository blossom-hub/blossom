...
let ``Balance Check 1`` () =
  let elts = [
    Posting(account1, Some ((3.4M, testCommodity), None), Some (CV account2), None, [])
  ]
  let f entry =
...
let ``Balance Check 2`` () =
  let elts = [
    Posting(account1, Some ((3.4M, testCommodity), None), None, None, [])
...
let ``Balance Check 3`` () =
  let elts = [
    Posting(account1, Some ((3.4M, testCommodity), None), None, None, [])
    Posting(account2, Some ((3.4M, Commodity "USD"), None), None, None, [])
